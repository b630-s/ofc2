...
TEST_NAME="${TEST_NAME:-dc-platform-access-smoke}"
...
create_role() {
  ...
  aws iam create-role \
    ...
    --tags "Key=Name,Value=${role_name}" "Key=Project,Value=dc-platform" "Key=Environment,Value=smoke" "Key=ManagedBy,Value=aws-cli" \
    --query 'Role.Arn' \
    --output text
}
...
create_smoke_test() {
  ...
  VPC_ID="$(
    aws_cli ec2 create-vpc \
      --cidr-block "${VPC_CIDR}" \
      --tag-specifications "ResourceType=vpc,Tags=[{Key=Name,Value=${TEST_NAME}-vpc},{Key=Project,Value=dc-platform},{Key=Environment,Value=smoke},{Key=ManagedBy,Value=aws-cli}]" \
      --query 'Vpc.VpcId' \
      --output text
  )"
  write_state
  ...
  INTERNET_GATEWAY_ID="$(
    aws_cli ec2 create-internet-gateway \
      --tag-specifications "ResourceType=internet-gateway,Tags=[{Key=Name,Value=${TEST_NAME}-igw},{Key=Project,Value=dc-platform},{Key=Environment,Value=smoke},{Key=ManagedBy,Value=aws-cli}]" \
  ...
  PUBLIC_SUBNET_1_ID="$(
    aws_cli ec2 create-subnet \
      ...
      --tag-specifications "ResourceType=subnet,Tags=[{Key=Name,Value=${TEST_NAME}-public-${AZ_1}},{Key=kubernetes.io/role/elb,Value=1},{Key=Project,Value=dc-platform},{Key=Environment,Value=smoke},{Key=ManagedBy,Value=aws-cli}]" \
  ...
  PUBLIC_SUBNET_2_ID="$(
    aws_cli ec2 create-subnet \
      ...
      --tag-specifications "ResourceType=subnet,Tags=[{Key=Name,Value=${TEST_NAME}-public-${AZ_2}},{Key=kubernetes.io/role/elb,Value=1},{Key=Project,Value=dc-platform},{Key=Environment,Value=smoke},{Key=ManagedBy,Value=aws-cli}]" \
      --query 'Subnet.SubnetId' \
      --output text
  )"
  PRIVATE_SUBNET_1_ID="$(
    aws_cli ec2 create-subnet \
      ...
      --tag-specifications "ResourceType=subnet,Tags=[{Key=Name,Value=${TEST_NAME}-private-${AZ_1}},{Key=kubernetes.io/role/internal-elb,Value=1},{Key=Project,Value=dc-platform},{Key=Environment,Value=smoke},{Key=ManagedBy,Value=aws-cli}]" \
      --query 'Subnet.SubnetId' \
      --output text
  )"
  PRIVATE_SUBNET_2_ID="$(
    aws_cli ec2 create-subnet \
      ...
      --tag-specifications "ResourceType=subnet,Tags=[{Key=Name,Value=${TEST_NAME}-private-${AZ_2}},{Key=kubernetes.io/role/internal-elb,Value=1},{Key=Project,Value=dc-platform},{Key=Environment,Value=smoke},{Key=ManagedBy,Value=aws-cli}]" \
      --query 'Subnet.SubnetId' \
      --output text
  )"
  write_state
  ...
  PUBLIC_ROUTE_TABLE_ID="$(
    aws_cli ec2 create-route-table \
      --vpc-id "${VPC_ID}" \
      --tag-specifications "ResourceType=route-table,Tags=[{Key=Name,Value=${TEST_NAME}-public-rt},{Key=Project,Value=dc-platform},{Key=Environment,Value=smoke},{Key=ManagedBy,Value=aws-cli}]" \
  ...
  aws_cli ec2 associate-route-table --route-table-id "${PUBLIC_ROUTE_TABLE_ID}" --subnet-id "${PUBLIC_SUBNET_1_ID}" >/dev/null
  aws_cli ec2 associate-route-table --route-table-id "${PUBLIC_ROUTE_TABLE_ID}" --subnet-id "${PUBLIC_SUBNET_2_ID}" >/dev/null

  log "Creating elastic IP and NAT gateway"
  ELASTIC_IP_ALLOCATION_ID="$(
    aws_cli ec2 allocate-address \
      --domain vpc \
      --tag-specifications "ResourceType=elastic-ip,Tags=[{Key=Name,Value=${TEST_NAME}-nat-eip},{Key=Project,Value=dc-platform},{Key=Environment,Value=smoke},{Key=ManagedBy,Value=aws-cli}]" \
      --query 'AllocationId' \
      --output text
  )"
  write_state
  ...
  NAT_GATEWAY_ID="$(
    aws_cli ec2 create-nat-gateway \
      ...
      --tag-specifications "ResourceType=natgateway,Tags=[{Key=Name,Value=${TEST_NAME}-nat},{Key=Project,Value=dc-platform},{Key=Environment,Value=smoke},{Key=ManagedBy,Value=aws-cli}]" \
  ...
  PRIVATE_ROUTE_TABLE_ID="$(
    aws_cli ec2 create-route-table \
      --vpc-id "${VPC_ID}" \
      --tag-specifications "ResourceType=route-table,Tags=[{Key=Name,Value=${TEST_NAME}-private-rt},{Key=Project,Value=dc-platform},{Key=Environment,Value=smoke},{Key=ManagedBy,Value=aws-cli}]" \
      --query 'RouteTable.RouteTableId' \
      --output text
  )"
  write_state
  ...
  aws_cli ec2 associate-route-table --route-table-id "${PRIVATE_ROUTE_TABLE_ID}" --subnet-id "${PRIVATE_SUBNET_1_ID}" >/dev/null
  aws_cli ec2 associate-route-table --route-table-id "${PRIVATE_ROUTE_TABLE_ID}" --subnet-id "${PRIVATE_SUBNET_2_ID}" >/dev/null
...
EOF
  aws_cli eks create-cluster \
    ...
    --tags "Name=${EKS_CLUSTER_NAME},Project=dc-platform,Environment=smoke,ManagedBy=aws-cli" \
    >/dev/null
  ...
  aws_cli eks create-nodegroup \
    ...
    --tags "Name=${NODEGROUP_NAME},Project=dc-platform,Environment=smoke,ManagedBy=aws-cli" \
    >/dev/null
  ...
  OIDC_PROVIDER_ARN="$(
    aws iam create-open-id-connect-provider \
      ...
      --tags "Key=Name,Value=${TEST_NAME}-oidc" "Key=Project,Value=dc-platform" "Key=Environment,Value=smoke" "Key=ManagedBy,Value=aws-cli" \
...
EOF
  ...
  aws_cli eks create-addon \
    ...
    --tags "Name=${TEST_NAME}-ebs-csi,Project=dc-platform,Environment=smoke,ManagedBy=aws-cli" \
    >/dev/null
...
destroy_smoke_test() {
  ...
  if [[ -n "${EBS_CSI_ROLE_NAME}" ]]; then
    ...
    delete_role_and_policies "${EBS_CSI_ROLE_NAME}" || true
  fi
  ...
  if [[ -n "${NODE_ROLE_NAME}" ]]; then
    ...
    delete_role_and_policies "${NODE_ROLE_NAME}" || true
  fi

  if [[ -n "${CLUSTER_ROLE_NAME}" ]]; then
    log "Deleting IAM role ${CLUSTER_ROLE_NAME}"
    delete_role_and_policies "${CLUSTER_ROLE_NAME}" || true
  fi

  if [[ -n "${NAT_GATEWAY_ID}" ]]; then
    log "Deleting NAT gateway ${NAT_GATEWAY_ID}"
    aws_cli ec2 delete-nat-gateway --nat-gateway-id "${NAT_GATEWAY_ID}" >/dev/null 2>&1 || true
    wait_for_nat_deletion "${NAT_GATEWAY_ID}" || true
  fi

  if [[ -n "${ELASTIC_IP_ALLOCATION_ID}" ]]; then
    log "Releasing elastic IP"
    aws_cli ec2 release-address --allocation-id "${ELASTIC_IP_ALLOCATION_ID}" >/dev/null 2>&1 || true
  fi

  if [[ -n "${INTERNET_GATEWAY_ID}" && -n "${VPC_ID}" ]]; then
    aws_cli ec2 detach-internet-gateway --internet-gateway-id "${INTERNET_GATEWAY_ID}" --vpc-id "${VPC_ID}" >/dev/null 2>&1 || true
    aws_cli ec2 delete-internet-gateway --internet-gateway-id "${INTERNET_GATEWAY_ID}" >/dev/null 2>&1 || true
  fi

  if [[ -n "${PUBLIC_SUBNET_1_ID}" ]]; then
    aws_cli ec2 delete-subnet --subnet-id "${PUBLIC_SUBNET_1_ID}" >/dev/null 2>&1 || true
  fi

  if [[ -n "${PUBLIC_SUBNET_2_ID}" ]]; then
    aws_cli ec2 delete-subnet --subnet-id "${PUBLIC_SUBNET_2_ID}" >/dev/null 2>&1 || true
  fi

  if [[ -n "${PRIVATE_SUBNET_1_ID}" ]]; then
    aws_cli ec2 delete-subnet --subnet-id "${PRIVATE_SUBNET_1_ID}" >/dev/null 2>&1 || true
  fi

  if [[ -n "${PRIVATE_SUBNET_2_ID}" ]]; then
    aws_cli ec2 delete-subnet --subnet-id "${PRIVATE_SUBNET_2_ID}" >/dev/null 2>&1 || true
  fi

  if [[ -n "${PUBLIC_ROUTE_TABLE_ID}" ]]; then
    aws_cli ec2 delete-route-table --route-table-id "${PUBLIC_ROUTE_TABLE_ID}" >/dev/null 2>&1 || true
  fi

  if [[ -n "${PRIVATE_ROUTE_TABLE_ID}" ]]; then
    aws_cli ec2 delete-route-table --route-table-id "${PRIVATE_ROUTE_TABLE_ID}" >/dev/null 2>&1 || true
  fi

  if [[ -n "${VPC_ID}" ]]; then
    aws_cli ec2 delete-vpc --vpc-id "${VPC_ID}" >/dev/null 2>&1 || true
  fi
