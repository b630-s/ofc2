#!/usr/bin/env bash
# dc-platform-bss smoke test script marker: v2026-04-23b
...
SCRIPT_VERSION="v2026-04-23b"
AWS_REGION="${AWS_REGION:-}"
CLUSTER_VERSION="${CLUSTER_VERSION:-1.34}"
NODE_INSTANCE_TYPE="${NODE_INSTANCE_TYPE:-t3.small}"
NODE_DISK_SIZE="${NODE_DISK_SIZE:-20}"

SHARED_CLUSTER_ROLE_NAME="${SHARED_CLUSTER_ROLE_NAME:-dc-platform-bss-eks-cluster-role}"
SHARED_NODE_ROLE_NAME="${SHARED_NODE_ROLE_NAME:-dc-platform-bss-eks-node-role}"
SHARED_EBS_CSI_ROLE_NAME="${SHARED_EBS_CSI_ROLE_NAME:-dc-platform-bss-ebs-csi-role}"
...
NODE_ROLE_CREATED="false"
EBS_CSI_ROLE_CREATED="false"
...
Defaults:
  ...
  node type            t3.small
  node disk            20 GiB
  state file           infra/aws-proto/.aws-access-smoke-test.env

Examples:
  ./infra/aws-proto/00-aws-access-smoke-test.sh create --region us-east-1
  ./infra/aws-proto/00-aws-access-smoke-test.sh status
  ./infra/aws-proto/00-aws-access-smoke-test.sh destroy
...
Notes:
  ...
  - If shared IAM roles already exist, this script reuses them and does not delete them.
...
PROVIDED_CLUSTER_ROLE_ARN="${PROVIDED_CLUSTER_ROLE_ARN}"
PROVIDED_NODE_ROLE_ARN="${PROVIDED_NODE_ROLE_ARN}"
CLUSTER_ROLE_CREATED="${CLUSTER_ROLE_CREATED}"
NODE_ROLE_CREATED="${NODE_ROLE_CREATED}"
EBS_CSI_ROLE_CREATED="${EBS_CSI_ROLE_CREATED}"
VPC_CIDR="${VPC_CIDR}"
...
update_role_trust_policy() {
  ...
  log "Updating trust policy for existing role ${role_name}"
...
build_names() {
  ...
  NODE_ROLE_NAME="${SHARED_NODE_ROLE_NAME}"
  EBS_CSI_ROLE_NAME="${SHARED_EBS_CSI_ROLE_NAME}"
...
create_smoke_test() {
  ...
  if [[ -n "${PROVIDED_CLUSTER_ROLE_ARN}" ]]; then
    log "Using explicitly provided EKS control plane IAM role ARN"
    CLUSTER_ROLE_ARN="${PROVIDED_CLUSTER_ROLE_ARN}"
    CLUSTER_ROLE_NAME="${CLUSTER_ROLE_ARN##*/}"
    CLUSTER_ROLE_CREATED="false"
    write_state
  elif role_exists "${CLUSTER_ROLE_NAME}"; then
    log "Checking IAM role ${CLUSTER_ROLE_NAME}: found"
    log "Reusing existing EKS control plane IAM role ${CLUSTER_ROLE_NAME}"
    CLUSTER_ROLE_ARN="$(role_arn "${CLUSTER_ROLE_NAME}")"
    CLUSTER_ROLE_CREATED="false"
    write_state
  else
    log "Checking IAM role ${CLUSTER_ROLE_NAME}: not found"
    log "Creating IAM role ${CLUSTER_ROLE_NAME} for the EKS control plane"
...
EOF
    ...
    write_state
    attach_managed_policy "${CLUSTER_ROLE_NAME}" arn:aws:iam::aws:policy/AmazonEKSClusterPolicy
  fi

  if [[ -n "${PROVIDED_NODE_ROLE_ARN}" ]]; then
    log "Using explicitly provided managed node group IAM role ARN"
    NODE_ROLE_ARN="${PROVIDED_NODE_ROLE_ARN}"
    NODE_ROLE_NAME="${NODE_ROLE_ARN##*/}"
    NODE_ROLE_CREATED="false"
    write_state
  elif role_exists "${NODE_ROLE_NAME}"; then
    log "Checking IAM role ${NODE_ROLE_NAME}: found"
    log "Reusing existing managed node group IAM role ${NODE_ROLE_NAME}"
    NODE_ROLE_ARN="$(role_arn "${NODE_ROLE_NAME}")"
    NODE_ROLE_CREATED="false"
    write_state
  else
    log "Checking IAM role ${NODE_ROLE_NAME}: not found"
    log "Creating IAM role ${NODE_ROLE_NAME} for the managed node group"
...
EOF
    ...
    NODE_ROLE_CREATED="true"
    write_state
    attach_managed_policy "${NODE_ROLE_NAME}" arn:aws:iam::aws:policy/AmazonEKSWorkerNodePolicy
    attach_managed_policy "${NODE_ROLE_NAME}" arn:aws:iam::aws:policy/AmazonEC2ContainerRegistryPullOnly
    attach_managed_policy "${NODE_ROLE_NAME}" arn:aws:iam::aws:policy/AmazonEKS_CNI_Policy
  fi
...
EOF
  ...
  # Amazon EKS OIDC endpoints are chained to the Starfield root CA.
  # Using the documented root thumbprint avoids fragile certificate parsing in CloudShell.
  local oidc_thumbprint="9e99a48a9960b14926bb7f3b02e22da0afd29abd"
...
EOF
  if role_exists "${EBS_CSI_ROLE_NAME}"; then
    log "Checking IAM role ${EBS_CSI_ROLE_NAME}: found"
    log "Reusing existing EBS CSI IAM role ${EBS_CSI_ROLE_NAME}"
    EBS_CSI_ROLE_ARN="$(role_arn "${EBS_CSI_ROLE_NAME}")"
    EBS_CSI_ROLE_CREATED="false"
    update_role_trust_policy "${EBS_CSI_ROLE_NAME}" "${TMP_DIR}/ebs-csi-trust.json"
    write_state
  else
    log "Checking IAM role ${EBS_CSI_ROLE_NAME}: not found"
    log "Creating IAM role ${EBS_CSI_ROLE_NAME} for the EBS CSI addon"
    EBS_CSI_ROLE_ARN="$(create_role "${EBS_CSI_ROLE_NAME}" "${TMP_DIR}/ebs-csi-trust.json")"
    EBS_CSI_ROLE_CREATED="true"
    write_state
    attach_managed_policy "${EBS_CSI_ROLE_NAME}" arn:aws:iam::aws:policy/service-role/AmazonEBSCSIDriverPolicy
  fi
...
destroy_smoke_test() {
  ...
  CLUSTER_ROLE_CREATED="${CLUSTER_ROLE_CREATED:-false}"
  NODE_ROLE_CREATED="${NODE_ROLE_CREATED:-false}"
  EBS_CSI_ROLE_CREATED="${EBS_CSI_ROLE_CREATED:-false}"
  ...
  if [[ -n "${EBS_CSI_ROLE_NAME}" && "${EBS_CSI_ROLE_CREATED}" == "true" ]]; then
    log "Deleting script-created IAM role ${EBS_CSI_ROLE_NAME}"
    delete_role_and_policies "${EBS_CSI_ROLE_NAME}" || log "WARNING: failed to delete IAM role ${EBS_CSI_ROLE_NAME}"
  elif [[ -n "${EBS_CSI_ROLE_NAME}" ]]; then
    log "Keeping reused/admin IAM role ${EBS_CSI_ROLE_NAME}"
  fi
  ...
  if [[ -n "${NODE_ROLE_NAME}" && "${NODE_ROLE_CREATED}" == "true" ]]; then
    log "Deleting script-created IAM role ${NODE_ROLE_NAME}"
    delete_role_and_policies "${NODE_ROLE_NAME}" || log "WARNING: failed to delete IAM role ${NODE_ROLE_NAME}"
  elif [[ -n "${NODE_ROLE_NAME}" ]]; then
    log "Keeping reused/admin IAM role ${NODE_ROLE_NAME}"
  fi

  if [[ -n "${CLUSTER_ROLE_NAME}" && "${CLUSTER_ROLE_CREATED}" == "true" ]]; then
    log "Deleting script-created IAM role ${CLUSTER_ROLE_NAME}"
    delete_role_and_policies "${CLUSTER_ROLE_NAME}" || log "WARNING: failed to delete IAM role ${CLUSTER_ROLE_NAME}"
  elif [[ -n "${CLUSTER_ROLE_NAME}" ]]; then
    log "Keeping reused/admin IAM role ${CLUSTER_ROLE_NAME}"
  fi
